#*********************************************
#*********************************************
#' Returns three dimensional rotation matrices for rotation around the x-axis of a three dimensional coordinate system.
#'
#' @param ang  is a vedctor of the rotation angles around the x-axis.
...
#' @rdname rotx
#'
rotx<-function(ang,radians=TRUE,block.out=FALSE,drop.out=TRUE,byrow=TRUE){
	...
	#
	############ VARIABLES: ############
	# ---ang--- is a vedctor of the rotation angles around the x-axis.
	...
	if(block.out){
		if(byrow){
			a=rbind(1,0,0,0,cos(ang),sin(ang),0,-sin(ang),cos(ang))
			dim(a)=c(3,3*l)
			a=t(a)
			}
		else{
