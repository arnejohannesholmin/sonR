...
#' @param segvar  is a vector of the variables to treat, named "segvar" for historical reasons (this function was first intended for, and possibly still is, segmentation data).
#'
#' @return
#'
#' @examples
...
#'
read.event_insertNA<-function(thisout, TIME, filesind, filelist, t, tlist, var, segvar){
	...
	#
	############ VARIABLES: ############
	# ---thisout--- is a list containing the relevant data.
	# ---TIME--- is the list returned from UNIX_time().
	# ---filesind--- is a vector of the indexes of the relevant files in the list of files.
	# ---filelist--- is the list of files.
	# ---t--- is the tine step indexes to return data at.
	# ---tlist--- is the list of time step indexes for the files.
	# ---segvar--- is a vector of the variables to treat, named "segvar" for historical reasons (this function was first intended for, and possibly still is, segmentation data).
	...
	insertNA = which(thisnumt>1 & thisnumt<length(t) & (names(thisout) %in% c(segvar,var)))
