...
write.noise.cruise<-function(
	# Main variables:
	con = NULL, esnm = NULL, treat = c("bgns","pdns","nrns","hins"), 
	...
	if("bgns" %in% treat){
		bgns = get.bgns.event(con = NULL, event = event_p, t = t_p, esnm = esnm, noise = noise, treat = treat, fftthreshold = fftthreshold, nsind = nsind, acfq = acfq, prob = prob, type = type, tries = tries, thr = thr, trim = trim, search = search, kurtosis = kurtosis, reffan = reffan, scale = scale, lowerpar = lowerpar, startpar = startpar, upperpar = upperpar, kern = kern, high = high)
		# Select only the bgns-variables to write/return:
		if(length(bgns)>0){
			bgns = bgns[bgnsnames]
			}
		if(any(is.na(bgns$bgns)) && "pdns" %in% treat){
	...
	if("nrns" %in% treat){
		nrnp = get.nrns.event(con = NULL, event = event_p, t = t_p, bgns = bgns, filter = filter, nsdn = nsdn, farv = farv, surface = NULL, esnm = esnm)
		...
		if(identical(event_p, event_a) && identical(t_p, t_a)){
			...
			nrna = get.nrns.event(con = NULL, event = event_a, t = t_a, bgns = bgns, filter = filter, nsdn = nsdn, farv = farv, surface = surface, esnm = esnm)
			}
	...
	if("hins" %in% treat){
		...
		events = list.files(dirname(dirname(dirname(event_p))), full.names = TRUE)
	...
	########## Output ##########
	if(isTRUE(con)){
		event_p = as.numeric(event.path(event = event_p, esnm = esnm)$eventnr)
		event_a = as.numeric(event.path(event = event_a, esnm = esnm)$eventnr)
		con = file.path(dirname(dirname(event_p)), "Noise", "Main")
		if(!file.exists(con)){
		...
		con = file.path(con, filename, ".beams")
		}
	# Define the output:
	bgns = c(bgns, nrnp, nrna, hins, list(fltr = filter, nsdn = nsdn, sfsi = surface, esnm = esnm))
