...
#'
#' @param data  is a list of elements named according to the TSD file format holding the following information: "esnm" (may be given separately), "lenb", "", "", "", "", "", "", "", "", "", "", "".
...
#'
get.specs.esnm<-function(data, var=c("rect", "offset", "region", "valid","ind", "direction"), esnm=NULL, ind=NULL, add=30, beamstypes=1){
	...
	if(strff("reg",var)){
		...
		if(sonR_implemented(esnm, c("MBS"))){
			region=cbind(theta=c(min(data$dira)-add,max(data$dira)+add),phi=c(min(data$dire)-add,max(data$dire)+add))
			}
			...
			stop(paste("Acoustical instrument \"",esnm,"\", specified by 'esnm', not implemented",sep=""))
			}
		else{
	...
	# (5) Return the valid beams based on tha value of 'beams':
