...
#' @param ang			A vector of angles defining the points on the ellipse, as seen in a polar coordinate system centered at 'origin'.
#' @param x				A vector of x-values at which the ellipse is caclulated.
#' @param y				A vector of y-values at which the ellipse is caclulated.
#' @param n				The length of 'ang' (if different from NULL) as a sequence between the first and the second element of 'ang'.
#' @param aspectcomp	Logical: If TRUE the points are attempted to be positioned evenly along the curve of the ellipse, and not by even angle increments.
#'
#' @return
#'
#' @examples
