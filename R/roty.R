...
#'
#' @param ang  is a vedctor of the rotation angles around the y-axis.
...
#' @rdname roty
#'
roty<-function(ang,radians=TRUE,block.out=FALSE,drop.out=TRUE,byrow=TRUE){
	...
	#
	############ VARIABLES: ############
	# ---ang--- is a vedctor of the rotation angles around the y-axis.
	...
	if(block.out){
		if(byrow){
			a=rbind(cos(ang),0,-sin(ang),0,1,0,sin(ang),0,cos(ang))
			dim(a)=c(3,3*l)
			a=t(a)
			}
		else{
